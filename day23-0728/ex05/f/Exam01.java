...
public class Exam01 {
  public static void main(String[] args) {
    ...
    B obj = new B(); // B클래스를 로딩해야 함 
                    //-> B클래스의 수퍼클래스는 A클래스네? A클래스 먼저 로딩 후, B클래스 로딩
    ...
    // B 클래스의 인스턴스 생성 과정
    // 1) B의 수퍼 클래스가 로딩되어 있지 않다면, B의 수퍼 클래스(A 클래스)를 먼저 로딩한다.
    //    - 스태틱 필드 생성한 후
    //    - 스태틱 블록 실행한다.
    // 2) 그런 후에 B 클래스를 로딩한다.
    //    - 스태틱 필드 생성한 후
...
// - B 클래스 입장에서, B클래스에게 상속해주는 A 클래스를 말한다.
//
// 서브클래스(sub class) = 자식클래스(child class)
// - A 클래스 입장에서, A클래스가 상속해주는 B클래스를 말한다.
//
// 즉, 수퍼클래스나 서브클래스는 상대적인 개념이다.
