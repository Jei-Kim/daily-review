...
public class Exam0110 {
  ...
  static class A {
    ...
    // => Heap 영역에 생성된다.
    // => static이 붙지 않는다.
    int v1; // 4바이트 int 값을 저장할 메모리를 만들라는 명령! // 변수 자체가 아니고 명령임
    boolean v2; // true/false 논리값을 저장할 메모리를 만들라는 명령!
    // 이 명령은 new 명령을 실행할 때 비로서 실행된다.
    //명령은 method area에 있다가 new명령을 통해 heap에 생성?
  }
  ...
  public static void main(String[] args) {
    ...
    // 인스턴스가 생성될 때 만들어지는 v1, v2를 변수를 "인스턴스 변수"라 부른다.(인스턴스 필드, A의 인스턴스, A의 객체)
    // 인스턴스는 하나 이상의 인스턴트 필드로 구성됨

  // 클래스(설계도) 안에 아무것도 없어도 new A()하면 힙에 인스턴스는 무조건 생성됨! 
  // -> 변수의 부가정보(어느 클래스에 생성되었는지 등)가 담겨서 저장됨.

    // 인스턴스 변수는 레퍼런스를 통해 사용할 수 있다.  
    obj1.v1 = 100; // 레퍼런스.필드명
    obj2.v1 = 200;
    obj3.v1 = 300;
